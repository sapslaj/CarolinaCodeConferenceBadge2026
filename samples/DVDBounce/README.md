...
- The real `DVD VIDEO` logo (88×53) bounces around the 128×160 display, changing to a new random-looking color each time it hits an edge.
...
`dvd_logo.png` is the full-resolution source (247×148, black on white). The BMP is derived from it: cropped, scaled to 88 px wide, and quantized to an 8-level **coverage ramp** — palette index 0 = no ink, index 7 = solid, 1–6 = the antialiased edges. The colors stored in the file are placeholders; `code.py` overwrites the whole ramp every frame.
